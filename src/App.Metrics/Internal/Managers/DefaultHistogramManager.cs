...
namespace App.Metrics.Internal.Managers
{
    internal class DefaultHistogramManager : IMeasureHistogramMetrics
    {
        ...
        public DefaultHistogramManager(IAdvancedMetrics advanced, IMetricsRegistry registry)
        {
            _advanced = advanced;
            _registry = registry;
        }
        ...
        {
            _registry.Histogram(options, () => _advanced.BuildHistogram(options)).Update(value);
        }

        /// <inheritdoc />
        public void Update(HistogramOptions options, long value, string userValue)
        {
            _registry.Histogram(options, () => _advanced.BuildHistogram(options)).Update(value, userValue);
        }
    }
}